    ...
    return (
      <section className="terminal-panel mb-12">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 p-5">
          ...
          <Link href="/ufc/ledger" className="rounded border border-accent/30 bg-accent/[0.07] px-3 py-2 text-[9px] uppercase tracking-[0.15em] text-accent transition-colors hover:border-accent/60 hover:bg-accent/10">
            Open ledger →
          </Link>
        </div>
      </section>
      ...
      <div className="terminal-panel">
        ...
        <div className="terminal-kpi-grid grid grid-cols-2 gap-px rounded-none border-x-0 border-t-0 lg:grid-cols-4">
          <div className="terminal-kpi px-4 py-3.5 sm:px-5">
            <div className="text-[9px] uppercase tracking-[0.18em] text-muted">Record</div>
            <div className="display mt-1 text-2xl font-bold text-zinc-100 tabnums">{graded.length ? `${hits}-${graded.length - hits}` : "—"}</div>
            <div className="mt-1 text-[9px] uppercase tracking-[0.12em] text-zinc-600">graded picks</div>
          </div>
          <div className="terminal-kpi px-4 py-3.5 sm:px-5">
            <div className="text-[9px] uppercase tracking-[0.18em] text-muted">Hit rate</div>
            <div className="display mt-1 text-2xl font-bold text-accent tabnums">
              {graded.length ? pct(hits / graded.length) : "—"}
            </div>
            <div className="mt-1 text-[9px] uppercase tracking-[0.12em] text-zinc-600 tabnums">{hits} correct</div>
          </div>
          <div className="terminal-kpi px-4 py-3.5 sm:px-5">
            <div className="text-[9px] uppercase tracking-[0.18em] text-muted">Model log loss</div>
            <div className="display mt-1 text-2xl font-bold text-accent tabnums">
              {llModel !== null ? llModel.toFixed(3) : "—"}
            </div>
            <div className="mt-1 text-[9px] uppercase tracking-[0.12em] text-zinc-600">lower is better</div>
          </div>
          <div className="terminal-kpi px-4 py-3.5 sm:px-5">
            <div className="text-[9px] uppercase tracking-[0.18em] text-muted">Books log loss</div>
              ...
              .map((e) => {
                ...
                return (
                  <div key={e.boutId} className="group flex flex-wrap items-center gap-3 px-4 py-3 transition-colors hover:bg-white/[0.025] sm:flex-nowrap sm:px-5">
                    <span className={`display inline-flex min-w-12 shrink-0 justify-center rounded border px-2 py-1 text-[9px] font-extrabold tracking-[0.12em] ${hit ? "border-accent/30 bg-accent/10 text-accent" : "border-danger/30 bg-danger/10 text-danger"}`}>
                      {hit ? "HIT" : "MISS"}
                    </span>
                    ...
                    <span className="ml-auto shrink-0 text-[9px] uppercase tracking-[0.12em] text-zinc-600">
                      Model pick
                    </span>
